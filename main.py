...
RISK_MAP = {
    ...
    1: {
        "label": "Moderate Risk",
        "color": "Orange",
        "advice": "Consider stress reduction techniques and better sleep."
    },
    2: {
        "label": "High Risk",
        "color": "Red",
        "advice": "Seek professional mental health support immediately."
    }
}
...
@app.route("/", methods=["GET"])
def home():
    return "Mental Health Risk API is running"
...
def predict():
    try:
        ...
        if extra:
            return jsonify({"error": "Unexpected features", "extra": extra}), 400
        ...
        for col, le in cat_encoders.items():
            if col in data:
                ...
                if value in le.classes_:
                    ...
                    return jsonify({
                        "error": f"Unknown category '{value}' for {col}"
                    }), 400
